...
| `migrate_message_type.py` | Added `messages.type`, Discord's own kind for a message. 4 is a thread rename, carrying the new name and whoever did it -- the only exact record of a retag there is, and 573 of production's messages parse as a title with 550 of those written by people, so a rename and a paste are otherwise the same row. Existing rows are left NULL: the type is not derivable from anything stored, only re-fetched, and `load_messages` fills it in on any row that has none. |
| `migrate_outbox_steps.py` | Added `sent_steps` to `events` and `new_threads`: which irreversible Discord writes a row has already had. Posting an event takes up to four writes and creating a ticket takes three, and only the last was ever recorded -- so a failure anywhere threw away the record of everything before it and the retry did it all again. Measured: an archive failing twice put three identical "marked this complete" messages in one thread, and an opening message failing twice left three real threads for one ticket. |
